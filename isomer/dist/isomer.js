...
/***/ function(module, exports, __webpack_require__) {
	...
	function Canvas(elem) {
	  ...
	  this.boundingClientRect = this.elem.getBoundingClientRect();
...
/***/ function(module, exports, __webpack_require__) {
	 ...
	 */
	Isomer.prototype.add = function(item, baseColor, lighten = true) {
	  if (Object.prototype.toString.call(item) == '[object Array]') {
	    for (var i = 0; i < item.length; i++) {
	      this.add(item[i], baseColor, lighten);
	    }
	  } else if (item instanceof Path) {
	    this._addPath(item, baseColor, lighten);
	  } else if (item instanceof Shape) {
	    /* Fetch paths ordered by distance to prevent overlaps */
	    var paths = item.orderedPaths();

	    for (var j = 0; j < paths.length; j++) {
	      this._addPath(paths[j], baseColor,lighten);
	    }
	  }
	};
	 ...
	 */
	Isomer.prototype._addPath = function(path, baseColor, shade = false) {
	  ...
	  //----- turn off the light effect -- added by RyanG----\/
	  var color;
	  if(shade){
